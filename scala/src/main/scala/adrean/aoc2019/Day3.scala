...
object Day3 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var step = 0
    var position = (0, 0)
    for (direction <- first.split(",")) {
      val length = direction.substring(1).toInt
      val action = directionActions(direction(0))
      for (_ <- 1 to length) {
        step += 1
        position = action.apply(position)
        if (grid(position) == 0) grid(position) = step
      }
    }

    step = 0
    position = (0, 0)
    for (direction <- second.split(",")) {
      val length = direction.substring(1).toInt
      val action = directionActions(direction(0))
      for (_ <- 1 to length) {
        step += 1
        position = action.apply(position)
        if (grid(position) > 0) grid(position) = -grid(position) - step
      }
    }

    println(grid.filter(_._2 < 0).maxBy(_._2))

    val closestByDistance = grid.filter(_._2 < 0).minBy(x => manhattanDistanceToOrigin(x._1))
    println(s"Part 1 : $closestByDistance => ${manhattanDistanceToOrigin(closestByDistance._1)}")

    val closestByStep = grid.filter(_._2 < 0).maxBy(_._2)
    println(s"Part 2 : $closestByStep}")
  }
}
